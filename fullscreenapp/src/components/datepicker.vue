...
.datetime-picker input {
    ...
    height: 22px;
    box-sizing: border-box;
    border: 2px solid rgba(0, 168, 255, 0);
    ...
    <div class="datetime-picker" :style="{ width: width }">
        ...
        <div class="picker-wrap" v-show="show">
            <table class="date-picker">
                ...
                <tbody>
                    <tr v-for="i in 5">
                        <td v-for="j in 7" :class="date[(i-1) * 7 + (j-1)] && date[(i-1) * 7 + (j-1)].status" :date="date[(i-1) * 7 + (j-1)] && date[(i-1) * 7 + (j-1)].date" @click="pickDate((i-1) * 7 + (j-1))">{{date[(i-1) * 7 + (j-1)] && date[(i-1)* 7 + (j-1)].text}}</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
export default {
    ...
    methods: {
        ...
        update() {
            ...
            for (let i = curFirstDay; i > 0; i--) {
                let passDay = lastDayCount - i + 1;
                let passTime = new Date(time.getFullYear(), time.getMonth(), passDay);
                arr.push({
                    text: passDay,
                    time: passTime,
                    status: 'date-pass'
    ...
    mounted() {
        ...
        var month = time.getMonth() + 1;
        var date = time.getDate();
        this.value = year + '-' + ((month < 10) ? ('0' + month) : month) + '-' + date;
    }
};
</script>